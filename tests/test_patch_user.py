...
class TestPatchUser:
    ...
    def test_valid_response(self):
        """Patch a new name/job for user 1 and assert it updates.

        Note: This will not actually change the value of ID 1 due to this being a public API
        """
        response = self.api.patch_user(1, self.new_name, self.new_job)
        response.validate()
        assert response.name == self.new_name
        assert response.job == self.new_job

    def test_valid_response_if_no_name(self):
        """Patch a new job with no name for user 1 and assert it updates.

        Note: This will not actually change the value of ID 1 due to this being a public API
        """
        response = self.api.patch_user(2, None, self.new_job)
        response.validate()
        assert response.name is None
        assert response.job == self.new_job

    def test_valid_response_if_no_job(self):
        """Patch a new name with no job for user 1 and assert it updates.

        Note: This will not actually change the value of ID 1 due to this being a public API
        """
        response = self.api.patch_user(3, self.new_name, None)
        response.validate()
        assert response.name == self.new_name
